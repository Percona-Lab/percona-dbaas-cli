...
Usage: $0 [OPTIONS]
    The following options may be given :
        ...
        --repo              Repo for build
        --kube_branch       Kubernetes branch for build
        --kube_repo         Kubernetes repo for build
        --version           Version to build
        --win               Build tarball for Windows
        --mac               Build tarball for mac

        --help) usage ;;
Example $0 --builddir=/tmp/percona_dbaas_cli --get_sources=1 --build_src_rpm=1 --build_rpm=1
EOF
        exit 1
}
...
parse_arguments() {
    ...
    for arg do
        ...
        case "$arg" in
            ...
            --branch=*) BRANCH="$val" ;;
            --repo=*) REPO="$val" ;;
            --kube_branch=*) KUBE_BRANCH="$val" ;;
            --kube_repo=*) KUBE_REPO="$val" ;;
            --version=*) VERSION="$val" ;;
...
get_sources(){
    ...
    echo "PRODUCT=${PRODUCT}" > percona-dbaas-cli.properties
    echo "BUILD_NUMBER=${BUILD_NUMBER}" >> percona-dbaas-cli.properties
    echo "BUILD_ID=${BUILD_ID}" >> percona-dbaas-cli.properties
    echo "VERSION=${VERSION}" >> percona-dbaas-cli.properties
    echo "BRANCH=${BRANCH}" >> percona-dbaas-cli.properties
    git clone "$REPO"
    retval=$?
    if [ $retval != 0 ]
    then
    ...
    then
        git reset --hard
        git clean -xdf
        git checkout "$KUBE_BRANCH"
    fi
    REVISION=$(git rev-parse --short HEAD)
    echo "VERSION=${VERSION}" > VERSION
    echo "REVISION=${REVISION}" >> VERSION
    echo "REVISION=${REVISION}" >> ${WORKDIR}/percona-dbaas-cli.properties
    rm -fr debian rpm
    cd ${WORKDIR}

    mv percona-dbaas-cli ${PRODUCT}-${VERSION}
    tar --owner=0 --group=0 --exclude=.* -czf ${PRODUCT}-${VERSION}.tar.gz ${PRODUCT}-${VERSION}
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT}-${VERSION}/${BRANCH}/${REVISION}/${BUILD_ID}" >> percona-dbaas-cli.properties
...
install_golang() {
    wget ${GO_TARBALL} -O /tmp/golang.tar.gz
    tar -zxf /tmp/golang.tar.gz
    rm -rf /usr/local/go /usr/local/go1.8 /usr/local/go1.9 /usr/local/go1.11 /usr/local/go1.12
    mv go /usr/local/
}
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      RHEL=$(rpm --eval %rhel)
      if [ "x${RHEL}" = "x8" ]; then
          yum -y install rpm-build zip rpmlint rpmdevtools golang
      else
          yum -y install epel-release
          yum -y install rpmbuild rpm-build epel-rpm-macros rpmlint rpmdevtools golang
      fi
      install_golang
    else
      export DEBIAN=$(lsb_release -sc)
      export ARCH=$(echo $(uname -m) | sed -e 's:i686:i386:g')
      if [ ${DEBIAN} = buster ]; then
          apt-get -y install gnupg2
      fi
      INSTALL_LIST="devscripts debhelper debconf pkg-config curl zip golang"
...
build_srpm(){
    ...
    packaging/rpm/percona-dbaas-cli.spec > rpmbuild/SPECS/percona-dbaas-cli.spec
...
build_rpm(){
    ...
    echo "RHEL=${RHEL}" >> percona-dbaas-cli.properties
    echo "ARCH=${ARCH}" >> percona-dbaas-cli.properties
...
build_deb(){
    ...
    export ARCH=$(echo $(uname -m) | sed -e 's:i686:i386:g')
    #
    echo "DEBIAN=${DEBIAN}" >> percona-dbaas-cli.properties
    echo "ARCH=${ARCH}" >> percona-dbaas-cli.properties
...
build_tarball(){
    ...
    TARFILE=$(basename $(find . -name 'percona-dbaas-cli*.tar.gz' | sort | grep -v "tools" | tail -n1))
    ...
    mv ${PSMDIR}/* build/src/github.com/Percona-Lab/percona-dbaas-cli/
    mv build/src/github.com/Percona-Lab/percona-dbaas-cli/kubernetes build/src/k8s.io/
    export PATH=/usr/local/go/bin:${PATH}
    export GOROOT="/usr/local/go/"
    export GOPATH=${PWD}/build
    export PATH="/usr/local/go/bin:${PATH}:${GOPATH}"
    ...
    if [ ${WIN} = 1 -o ${MAC} = 1 ]; then
        zip -r ${WORKDIR}/${PSMDIR}-${ARCH}.zip ${PSMDIR} 
    else
    ...
    cp ${WORKDIR}/${PSMDIR}-${ARCH}${TARBALL_SUFFIX}.* ${WORKDIR}/${DIRNAME}
    cp ${WORKDIR}/${PSMDIR}-${ARCH}${TARBALL_SUFFIX}.* ${CURDIR}/${DIRNAME}
}
...
KUBE_BRANCH="v1.14.1"
KUBE_REPO="https://github.com/kubernetes/kubernetes"
GO_TARBALL="https://dl.google.com/go/go1.12.4.linux-amd64.tar.gz"
PRODUCT=percona-dbaas-cli
